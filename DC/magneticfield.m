...
m = 2;         % Mode number in X-Direction
n = 2;         % Mode number in Y-Direction
Bmn = 1;       % Amplitude
...
% Magnetic Field Expression for TMmn
Hx = Bmn*(By/epsilon)*sin(m*pi.*x./a).*cos(n*pi.*y./b).*exp(-1i*Bz*z);
Hy = -Bmn*(Bx/epsilon)*cos(m*pi.*x./a).*sin(n*pi.*y./b).*exp(-1i*Bz*z);
Hz = zeros(size(real(Hy)));
% Electric Field Expression for TMmn
Ex = -Bmn*(Bx*By/(omega*mu1*epsilon))*cos(m*pi.*x./a).*sin(n*pi.*y./b).*exp(-1i*Bz*z);
Ey = -Bmn*(By*Bz/(omega*mu1*epsilon))*cos(m*pi.*x./a).*sin(n*pi.*y./b).*exp(-1i*Bz*z);
Ez = -1i*Bmn*(Bc.^2/(omega*mu1*epsilon))*cos(m*pi.*x./a).*cos(n*pi.*y./b).*exp(-1i*Bz*z);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
figure();
